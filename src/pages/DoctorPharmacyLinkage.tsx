...
import {
  ...
  Select, TextInput, ActionIcon, Tooltip, Paper,
  SimpleGrid, Skeleton, ThemeIcon
} from '@mantine/core';
import { notifications } from '@mantine/notifications';
import {
  IconSearch, IconLink, IconUnlink, IconStethoscope, IconPrescription,
  IconCurrencyRupee, IconDownload, IconFilter, IconEdit,
  IconCheck, IconX, IconArrowLeft, IconSortAscending, IconSortDescending
...
type SortField = 'pharmacyName' | 'doctorName' | 'totalBusiness' | 'productCount';
...
export default function DoctorPharmacyLinkage() {
  ...
  const [sortField, setSortField] = useState<SortField>('totalBusiness');
    ...
    results.sort((a, b) => {
      ...
      switch (sortField) {
        ...
        case 'doctorName':
          ...
          break;
        case 'totalBusiness':
          cmp = a.totalBusiness - b.totalBusiness;
          break;
        case 'productCount':
  ...
  const handleUnlink = async (pharmacyId: number, pharmacyName: string) => {
    if (!window.confirm(`Remove doctor link from ${pharmacyName}?`)) return;
    try {
      const result = await api.put('/api/doctors/linkage', {
        pharmacyId,
        doctorId: null,
  ...
  const handleExportCSV = () => {
    const csvData = processedData.map((row, idx) => ({
      ...
      'Sale Qty': row.totalSaleQty,
      'Status': row.isDraft ? 'Draft' : 'Verified',
    }));
    exportToCSV(csvData, `doctor_pharmacy_linkage_${new Date().toISOString().split('T')[0]}`, ['#', 'Pharmacy', 'Doctor', 'Specialization', 'Total Business (₹)', 'Products', 'Sale Qty', 'Status']);
  };
  ...
  const stats = useMemo(() => {
    ...
    const totalBusiness = linkages.reduce((s, l) => s + l.totalBusiness, 0);
    const totalDoctors = new Set(linkages.filter(l => l.doctorId).map(l => l.doctorId)).size;
    return { linked, unlinked, totalBusiness, totalDoctors, totalPharmacies: linkages.length };
  }, [linkages]);
    ...
    return (
      <div className={styles.container}>
        ...
        <SimpleGrid cols={{ base: 2, md: 4 }} mb="xl">
          {[1, 2, 3, 4].map(i => <Skeleton key={i} height={80} radius="lg" />)}
        </SimpleGrid>
  ...
  return (
    <div className={styles.container}>
      ...
      {/* Summary Stats */}
      <SimpleGrid cols={{ base: 2, md: 5 }} mb="xl" mt="lg">
        <Paper className={styles.statCard} p="md" radius="lg" withBorder>
          <Group gap="sm">
            ...
            <div>
              <Text size="xs" c="dimmed" tt="uppercase" fw={600}>Total Business</Text>
              <Text fw={800} size="lg">₹{stats.totalBusiness.toLocaleString('en-IN')}</Text>
            </div>
          </Group>
        </Paper>
        <Paper className={styles.statCard} p="md" radius="lg" withBorder>
          <Group gap="sm">
            ...
            <div>
              <Text size="xs" c="dimmed" tt="uppercase" fw={600}>Doctors</Text>
              <Text fw={800} size="lg">{stats.totalDoctors}</Text>
            </div>
          </Group>
        </Paper>
        <Paper className={styles.statCard} p="md" radius="lg" withBorder>
          <Group gap="sm">
            ...
            <div>
              <Text size="xs" c="dimmed" tt="uppercase" fw={600}>Pharmacies</Text>
            ...
            <Table.Thead className={styles.tableHeader}>
              <Table.Tr>
                ...
                <Table.Th
                  ...
                  onClick={() => toggleSort('totalBusiness')}
                >
                  <Group gap={4} wrap="nowrap" justify="flex-end">Total Business <SortIcon field="totalBusiness" /></Group>
                </Table.Th>
                <Table.Th
                  style={{ cursor: 'pointer', textAlign: 'right' }}
                  onClick={() => toggleSort('productCount')}
                >
                  <Group gap={4} wrap="nowrap" justify="flex-end">Products <SortIcon field="productCount" /></Group>
                </Table.Th>
                ...
                processedData.map((row, idx) => (
                  <Table.Tr key={row.pharmacyId}>
                    ...
                    <Table.Td style={{ textAlign: 'right' }}>
                      <Text fw={700} size="sm" c={row.totalBusiness > 0 ? 'green' : 'dimmed'}>
                        ₹{row.totalBusiness.toLocaleString('en-IN')}
                      </Text>
                    </Table.Td>
                    <Table.Td style={{ textAlign: 'right' }}>
                      <Badge variant="light" color="violet" size="sm">{row.productCount}</Badge>
                    </Table.Td>
                    <Table.Td style={{ textAlign: 'right' }}>
                      <Text size="sm">{row.totalSaleQty.toLocaleString()}</Text>
                    </Table.Td>
                    <Table.Td>
                      {row.isDraft ? (
