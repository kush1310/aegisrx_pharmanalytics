...
const addBrandedHeader = (doc: jsPDF, title: string, subtitle: string) => {
  ...
  doc.text('SuratPharma Analytics', 14, 10);
...
const addFooter = (doc: jsPDF) => {
  ...
  for (let pageIndex = 1; pageIndex <= pageCount; pageIndex++) {
    ...
    doc.text('SuratPharma Analytics  |  Confidential', 14, pageHeight - 10);
...
export const exportAnalyticsPDF = (
  ...
  autoTable(doc, {
    ...
    body: pharmacyData.map((pharmacyItem, rowIndex) => [
      rowIndex + 1,
      pharmacyItem.name,
      `₹${pharmacyItem.revenue.toLocaleString('en-IN')}`,
  ...
  if (productData && productData.length > 0) {
    ...
    autoTable(doc, {
      ...
      body: productData.map((productItem, rowIndex) => [
        ...
        `₹${productItem.revenue.toLocaleString('en-IN')}`,
...
export const exportProfilePDF = (
  ...
  if (personalEntries.length > 0) {
    personalEntries.forEach(entry => {
      ...
      doc.setFontSize(10);
      doc.setTextColor(...TEXT_PRIMARY);
      doc.text(entry.value, 50, currentY);
      currentY += 8;
    });
  } else {
    doc.setFontSize(9);
  ...
  addFooter(doc);
  doc.save(`SuratPharma_Dr_${doctor.name.replace(/\s+/g, '_')}.pdf`);
};
...
export const exportPharmacyPDF = (
  ...
  doc.setFontSize(10);
  doc.setTextColor(...TEXT_PRIMARY);
  doc.text(pharmacy.ownerName, 60, currentY);
  currentY += 8;
  ...
  addFooter(doc);
  doc.save(`SuratPharma_${pharmacy.name.replace(/\s+/g, '_')}.pdf`);
};
...
export const exportDoctorListPDF = (
  ...
  addFooter(doc);
  doc.save(`SuratPharma_Doctors_Directory_${new Date().toISOString().split('T')[0]}.pdf`);
};
...
export const exportPharmacyListPDF = (
  ...
  addFooter(doc);
  doc.save(`SuratPharma_Pharmacies_Directory_${new Date().toISOString().split('T')[0]}.pdf`);
};
...
export const exportProductListPDF = (
  ...
  addFooter(doc);
  doc.save(`SuratPharma_Products_Directory_${new Date().toISOString().split('T')[0]}.pdf`);
};
